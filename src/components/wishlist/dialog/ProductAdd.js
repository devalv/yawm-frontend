...
export default function ProductAddFormDialog({props}) {
  ...
  const [errorState, setAddErrorState] = React.useState({ type: "", hasError: false});
  ...
  const handleSubmit = async (event) => {
    ...
    await axios.put(addEndpoint, data).then(() => {
      setAddErrorState({type: "", hasError: false  });
      handleClose();
  ...
  if (AuthState.authenticated) {
    ...
    <Dialog open={props.open} onClose={handleClose} component="form" onSubmit={handleSubmit}>
        ...
        <DialogActions>
          ...
          <Button type="submit" variant="outlined" endIcon={<SendIcon />}>Добавить</Button>
        </DialogActions>
      </Dialog>
  );
  }
